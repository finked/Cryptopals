...
func expandKey(key []byte, steps int) []byte {
	/*
	 *	rotate 4th column (4x4 key) n steps up
	 */
	res := make([]byte, len(key))
	copy(res, key)
	// fmt.Println(len(key))
	for i := 0; i < 4; i++ {
		res[i*4+3] = key[((i+steps)*4+3)%len(key)]
	}
	return res
}

func Sbox(key []byte) []byte {
	sBox := [256]byte{
